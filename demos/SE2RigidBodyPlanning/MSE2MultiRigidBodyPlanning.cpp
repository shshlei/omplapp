...
void envMaze(bool optimal, bool default_param, app::SE2MultiRigidBodyPlanning &setup, double &range, double &pen_distance)
{
    ...
    if (optimal)
    {
        ...
        obj->setCostThreshold(base::Cost(300.0));
...
void envBarriers(bool optimal, bool /*default_param*/, app::SE2MultiRigidBodyPlanning &setup, double &range, double &pen_distance)
{
    // load the robot and the environment
...
int main(int argc, char* argv[])
{
    ...
    for (unsigned int i = 0; i < run_cycle; i++)
    {
        ...
        {
            std::ofstream out(boost::str(boost::format("pathmultisol_%i.txt") % i).c_str());
            setup.getSolutionPath().printAsMatrix(out);
            out.close();
        }

        base::PlannerData pd(std::make_shared<ob::SpaceInformation>(setup.getGeometricComponentStateSpace()));
        setup.getPlannerData(pd, 0);
        pds.store(pd, boost::str(boost::format("pdmultisol_0_%i.txt") % i).c_str());

        pd.clear();
        setup.getPlannerData(pd, 1);
        pds.store(pd, boost::str(boost::format("pdmultisol_1_%i.txt") % i).c_str());
    }
