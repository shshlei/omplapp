...
bool argParse(int argc, char** argv, std::string &env, std::string &robot, std::string &spaceTypecheckResolution, bool &optimal, double &optimalT);
...
int main(int argc, char* argv[])
{
    ...
    if (!argParse(argc, argv, env, robot, spaceType, optimal, optimalT)) return -1;
    ...
    svc->setEnvironmentFile(env);
    svc->addRobotShape(robot);
...
bool argParse(int argc, char** argv, std::string &env, std::string &robot, std::string &spaceType, bool &optimal, double &optimalT)
{
    ...
    desc.add_options()
        ...
        ("spacetype,s", bpo::value<std::string>()->default_value("SE2"), "(Optional) Specify the planning space type, default to SE2 if not given. Valid options are SE2, Dubins, ReedsShepp.")
